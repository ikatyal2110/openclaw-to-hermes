...
def _classify(node: Node) -> Portability:
    ...
    if kind == NodeKind.PROMPT.value:
        return Portability(score=1.0, tier=PortabilityTier.PORTABLE, rationale="Prompt copied verbatim.")
    if kind == NodeKind.ENV.value:
        return Portability(score=1.0, tier=PortabilityTier.PORTABLE, rationale="Environment variable, passed through.")
    ...
    return Portability(score=0.0, tier=PortabilityTier.NEEDS_REVIEW, rationale=f"No classifier for kind={kind}.")
...
def _classify_workflow(node: Node, meta: dict) -> Portability:
    ...
    return Portability(score=0.95, tier=PortabilityTier.PORTABLE, rationale="Linear sequenceable workflow.")
...
def _classify_tool(node: Node, meta: dict) -> Portability:
    ...
    if kind_field == "router":
        return Portability(
            ...
            blockers=["Router has no one-to-one Hermes mapping; review the generated skill's `when_to_use`."],
        )
    if runtime == "http":
        return Portability(score=1.0, tier=PortabilityTier.PORTABLE, rationale="HTTP tool — direct map.")
    if runtime == "python":
        if meta.get("pure"):
            return Portability(score=0.8, tier=PortabilityTier.PORTABLE, rationale="Pure Python function — map to Hermes tool with a stub.")
        return Portability(
            score=0.4,
            tier=PortabilityTier.NEEDS_REVIEW,
    ...
    if any(se.kind == SideEffectKind.UNKNOWN.value or se.kind == SideEffectKind.UNKNOWN for se in node.side_effects):
        return Portability(score=0.3, tier=PortabilityTier.NEEDS_REVIEW, rationale="Unknown side-effect surface.")
    return Portability(score=0.5, tier=PortabilityTier.PARTIAL, rationale=f"Runtime {runtime!r} not recognized.")
...
def _classify_memory(node: Node, meta: dict) -> Portability:
    ...
    if kind == "kv":
        return Portability(score=1.0, tier=PortabilityTier.PORTABLE, rationale="KV store — direct map.")
    if kind == "vector":
        return Portability(
            score=0.6,
            tier=PortabilityTier.NEEDS_REVIEW,
    ...
    return Portability(score=0.3, tier=PortabilityTier.NEEDS_REVIEW, rationale=f"Unknown memory kind {kind!r}.")


def _classify_scheduler(node: Node, meta: dict) -> Portability:
    if meta.get("trigger_kind") == "cron":
        return Portability(score=1.0, tier=PortabilityTier.PORTABLE, rationale="Cron — passes through.")
    ...
    return Portability(score=0.4, tier=PortabilityTier.NEEDS_REVIEW, rationale="Unrecognized trigger kind.")
