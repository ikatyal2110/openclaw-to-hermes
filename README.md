...
- **v0.3 (current).** `praxis explain <node>` for drilling into classifications. Robust analyzer error handling (broken YAML → diagnostic, not crash). Secrets classifier on env vars (🔐 in the report). Real-project walkthrough doc.
- **v0.4.** LLM-assisted intent inference with content-addressed caching. Memory schema beyond KV/vector. Round-trip tests (Hermes → IR → Hermes).
- **v0.5.** Hybrid bridge, read-only (Hermes introspects OpenClaw tools).
- **v0.6.** Hybrid bridge, read-write. LangGraph as a third target — first chance to break the IR.
- **v0.7.** VS Code extension surfacing the migration report as inline annotations.
